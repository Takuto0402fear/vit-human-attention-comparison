"""
Experiment A (DINO ViT-B/16 full700): official DINO ViT-B/16 layerwise
[CLS]->patch attention vs. human fixations (OSIE, healthy) -- full
700-image production run, at the SAME Base model scale as the existing
CLIP ViT-B/16 experiment.

Same evaluation convention as scripts/run_expA.py (DINO-S) and
scripts/run_expA_clip_full700.py (CLIP-B): bilinear upsample to 800x600,
metrics.py's nss/auc_judd/sauc imported unmodified, identical fixation-
coordinate extraction, identical center-Gaussian/uniform-noise positive
controls, identical sAUC negative-pool convention (one shared 700-image
pool, no self-image exclusion, metrics.py's internal RandomState(0)
untouched). Uses ONLY the already-validated memory-conscious extractor
(dino_vitb16_extractor.py::extract_layerwise_cls_attention_memory_conscious),
unmodified -- no speed/refactor changes this stage.

Separate script from scripts/run_expA_dino_vitb16.py (the 20-image
pilot, validated and committed at 67322d5): that script, run_expA.py,
run_expA_clip.py, run_expA_clip_full700.py, dino_vitb16_extractor.py,
vit_extractor.py, and clip_extractor.py are all untouched by this run.

Guards: refuses to run if any FINAL output already exists, or if a
partial file from a previous attempt is present (no silent overwrite,
no silent resume). Checkpoints metrics_per_image.partial.csv every 50
images (flush + fsync); partial files are left in place as an audit
trail, not deleted, and are never treated as the official result. No
attention cache is written this stage.

Output confined to outputs/expA_dino_vitb16/healthy/ (NOT pilot20/).
Does not touch outputs/expA/, outputs/expA_clip/,
outputs/expA_dino_vitb16/pilot20/, outputs/attn_cache/, or results/.

Usage (PowerShell):
    python scripts\\run_expA_dino_vitb16_full700.py
"""
import sys
...
PROJECT_DIR = r"C:\Users\user\gaze"
# ======================================================
...
def _get_git_commit():
    try:
        return subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=PROJECT_DIR, text=True).strip()
    except Exception:
        return None
...
def main():
    ...
    print("=" * 65)
    print("  Exp A (DINO ViT-B/16 full700): DINO-B vs Human Fixations")
    print("=" * 65)

    execution_base_commit = _get_git_commit()
    ...
    with open(PARTIAL_CONFIG, "w", encoding="utf-8") as f:
        json.dump({
            "status": "in_progress", "model": MODEL, "execution_base_commit": execution_base_commit,
            "n_images_target": N_EXPECTED, "image_ids": image_ids,
    ...
    # ------------------------------------------------------------------
    print(f"\n--- Extracting DINO-B attention + computing metrics ({N_EXPECTED} images) ---")
    if device == "cuda":
    ...
    for idx, img_id in enumerate(image_ids):
        ...
        if n_done % CHECKPOINT_EVERY == 0 or n_done == N_EXPECTED:
            ...
            peak_alloc_so_far = (torch.cuda.max_memory_allocated() / 1024 ** 2) if device == "cuda" else None
            peak_reserved_so_far = (torch.cuda.max_memory_reserved() / 1024 ** 2) if device == "cuda" else None
            with open(PARTIAL_TIMING, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "peak_vram_allocated_mib_so_far": peak_alloc_so_far,
                    "peak_vram_reserved_mib_so_far": peak_reserved_so_far,
                }, f, indent=2)
    ...
    print("\n=== metrics_by_layer (DINO ViT-B/16, full700) ===")
    ...
    for ax, (mname, mc, sc) in zip(axes, plot_defs):
        ...
        ax.plot(layers_arr, means, "o-", color="seagreen", lw=2, ms=5, label="DINO ViT-B/16")
        ax.fill_between(layers_arr, means - stds, means + stds, alpha=0.15, color="seagreen")
        ...
        ax.set_xlabel("Layer"); ax.set_ylabel(mname if mname != "AUC_Judd" else "AUC-Judd")
        ax.set_title(mname if mname != "AUC_Judd" else "AUC-Judd")
        ax.set_xticks(layers_arr); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    fig.suptitle(f"Exp-A (DINO ViT-B/16): DINO-B vs Human Fixations ({GROUP}, full700)", fontsize=13)
    fig.tight_layout()
    ...
    # ------------------------------------------------------------------
    run_config = {
        "model": MODEL, "execution_base_commit": execution_base_commit,
    ...
    print("\n" + "=" * 65)
    print("  Exp A (DINO ViT-B/16 full700): DONE")
    print("  outputs/expA_dino_vitb16/pilot20/, outputs/expA/, outputs/expA_clip/, "
